defmodule RumblWeb.VideoControllerTest do
  ...
  test "requires user authentication on all actions", %{conn: conn} do
    Enum.each([
      ...
      put(conn, Routes.video_path(conn, :update, "234", %{})),
      post(conn, Routes.video_path(conn, :create, %{})),
      delete(conn, Routes.video_path(conn, :delete, "234"))
    ], fn conn ->
  ...
  describe "with a logged-in user" do
    ...
    @tag login_as: "max"
    test "doesn't create video and renders error when invalid", %{conn: conn, user: _user} do
